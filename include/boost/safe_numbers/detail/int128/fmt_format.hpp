...
#ifndef BOOST_SAFE_NUMBERS_DETAIL_INT128_FMT_FORMAT_HPP
#define BOOST_SAFE_NUMBERS_DETAIL_INT128_FMT_FORMAT_HPP
...
#define BOOST_SAFE_NUMBERS_DETAIL_INT128_HAS_FMT_FORMAT
...
constexpr auto parse_impl(ParseContext& ctx)
{
    ...
    if (it != ctx.end() && *it != '}')
    {
        switch (*it++)
        {
            ...
            case 'X':
                ...
                // LCOV_EXCL_START
            default:
                BOOST_SAFE_NUMBERS_DETAIL_INT128_THROW_EXCEPTION(std::logic_error("Unsupported format specifier"));
                // LCOV_EXCL_STOP
        }
    }
    ...
    if (it != ctx.end() && *it != '}')
    {
        BOOST_SAFE_NUMBERS_DETAIL_INT128_THROW_EXCEPTION(std::logic_error("Expected '}' in format string")); // LCOV_EXCL_LINE
    }
...
struct formatter
{
    ...
    auto format(T v, FormatContext& ctx) const
    {
        ...
        BOOST_SAFE_NUMBERS_DETAIL_INT128_IF_CONSTEXPR (std::is_same<T, boost::int128::int128_t>::value)
        {
            if (v < T{0})
            {
                isneg = true;
...
#endif // BOOST_SAFE_NUMBERS_DETAIL_INT128_FMT_FORMAT_HPP
