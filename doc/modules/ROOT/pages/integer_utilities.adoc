...
----
template <non_bounded_unsigned_library_type T>
consteval auto ipow(const verified_type_basis<T> a, const verified_type_basis<T> b) noexcept -> T;
----

Compile-time only overload for verified types.
Returns the base safe integer type `T` (not a verified type).

Since `ipow` is `consteval` for verified types, the computation is guaranteed to occur at compile time.
...
constexpr auto r = ipow(verified_u32{u32{2}}, verified_u32{u32{20}});  // r == u32{1048576}
constexpr auto s = ipow(verified_u64{u64{10}}, verified_u64{u64{18}}); // s == u64{1000000000000000000}
----
