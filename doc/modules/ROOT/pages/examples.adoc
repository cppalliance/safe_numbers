...
[#examples_verified_mixed_ops]
== Verified Types: Mixed-Type Operations

Verified types are compile-time constants, but they can participate in runtime expressions when mixed with their basis types.
The result is always the runtime (basis) type, treating the verified value as a read-only constant operand.
Arithmetic, comparisons, and bitwise operations are all supported.
