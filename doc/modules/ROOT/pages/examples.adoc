...
[#examples_integer_utilities]
== Integer Utilities

The library provides utility functions for the safe integer types: integer square root, trailing-zero removal, power-of-N predicates, integer logarithms, exponentiation, and alignment helpers.
...
The library provides overloads of the standard `<numeric>` algorithms (`gcd`, `lcm`, `midpoint`) for the safe integer types.
...
The safe floating-point types `f32` and `f64` wrap `float` and `double`, running each of the four checked operators (`pass:[+]`, `-`, `pass:[*]`, `/`) and turning an exceptional IEEE 754 result into an exception: saturation to positive or negative infinity becomes `std::overflow_error` or `std::underflow_error`, and the invalid operations, a NaN operand, and division by zero become `std::domain_error`.

.This https://github.com/boostorg/safe_numbers/blob/develop/examples/floats.cpp[example] demonstrates arithmetic, comparison, and exception behavior with the safe floating-point types.
====
[source, c++]
----
include::example$floats.cpp[]
----
...
static_cast<double>(a * b) = 12

a < b        = true
nan < b      = false
nan == nan   = false

max + max threw std::overflow_error
(-max) + (-max) threw std::underflow_error
a / 0.0 threw std::domain_error
nan + a threw std::domain_error
----
====
...
The library wraps the standard `<cmath>` functions for the safe floating-point types `f32` and `f64`, re-checking each result against the IEEE 754 safety contract so that an exceptional result becomes an exception instead of being silently propagated.
...
The library specializes `std::hash` for every safe number type, so they can be used as keys in the standard unordered containers (`std::unordered_set`, `std::unordered_map`, and their variants).
