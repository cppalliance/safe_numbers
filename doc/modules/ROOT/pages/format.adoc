...
The integer format specifiers documented below apply to the integer types; for `f32` and `f64` the standard floating-point specifiers are forwarded to the underlying `float` or `double` formatter, as described in xref:#format_floating_point[Floating-Point Format Specifiers].
...
| (default) | Shortest representation that round-trips to the same value
| `f` | Fixed-point notation
| `e` | Scientific notation
| `g` | General: fixed or scientific, whichever is shorter
