...
seen.insert(u32{7});
seen.insert(u32{8});
bool has_seven = seen.count(u32{7}) == 1;   // true

std::unordered_map<i64, std::string> labels {};
labels[i64{-1}] = "minus one";

// The hash matches the hash of the underlying built-in value
auto h1 = std::hash<u32>{}(u32{42});
auto h2 = std::hash<std::uint32_t>{}(42u);
bool same = h1 == h2;                        // true

// Floating-point keys hash exactly as the built-in does
auto hf = std::hash<f64>{}(f64{3.14});       // == std::hash<double>{}(3.14)
----
