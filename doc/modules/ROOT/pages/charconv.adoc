...
The library provides character conversion functions for all safe integer types and the safe floating-point types using https://www.boost.org/doc/libs/master/libs/charconv/doc/html/charconv.html[Boost.Charconv].
These functions convert between the safe types and their string representations.
The integer overloads (covering `u8` through `u128`, `i8` through `i128`, and `bounded_uint`) take a numeric base, while the floating-point overloads (covering `f32`, `f64`, and `bounded_float`) take a `boost::charconv::chars_format` and an optional precision.
...
// Integer conversions (take a numeric base)
...
// Floating-point conversions (take a chars_format and optional precision)
template <safe_numbers::detail::float_library_type T>
auto to_chars(char* first, char* last,
              T value,
              chars_format fmt = chars_format::general) -> charconv::to_chars_result;

template <safe_numbers::detail::float_library_type T>
auto to_chars(char* first, char* last,
              T value,
              chars_format fmt, int precision) -> charconv::to_chars_result;

template <safe_numbers::detail::float_library_type T>
auto from_chars(const char* first, const char* last,
                T& value,
                chars_format fmt = chars_format::general) -> charconv::from_chars_result;
...
For the floating-point types `f32`, `f64`, and `bounded_float<Min, Max>`, separate `to_chars` and `from_chars` overloads take a `boost::charconv::chars_format` instead of a numeric base.
They delegate to the corresponding Boost.Charconv floating-point routines on the underlying `float` or `double`.
...
The `fmt` argument selects the textual representation:

|===
| `chars_format` | Meaning

| `chars_format::scientific` | Scientific notation, for example `1.5e+10`
| `chars_format::fixed` | Fixed notation, for example `15000000000`
| `chars_format::hex` | Hexadecimal floating-point notation
| `chars_format::general` | The shorter of fixed and scientific (the default)
|===

The third `to_chars` overload additionally takes a `precision`, the number of digits to emit (significant or fractional, depending on `fmt`).
...
| `first`, `last` | Character buffer to write to (`to_chars`) or character range to parse (`from_chars`)
| `value` | The safe floating-point value to convert, or the destination to parse into
| `fmt` | The `boost::charconv::chars_format` controlling the representation
| `precision` | Number of digits to emit (third `to_chars` overload only)
|===

The result types are the same `to_chars_result` and `from_chars_result` documented above.

NOTE: Unlike the integer overloads, the floating-point overloads are not `constexpr` and are not available in device code.
