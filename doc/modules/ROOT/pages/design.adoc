...
The same principles extend to the floating-point types `f32` and `f64`, which wrap `float` and `double` as concrete, named types under the identical rules on explicit conversion and disallowed mixed-width operations: `f32 + f64` is a compile-time error, and construction from any type other than the exact underlying type is rejected.
