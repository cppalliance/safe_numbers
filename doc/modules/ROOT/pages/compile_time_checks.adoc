...
* *Compiler and optimization.* GCC or Clang with optimization enabled (`-O1` or higher). It is inert at `-O0` and on MSVC, and never changes program behavior.
* *GCC* reports every case as a clean compile error that names the condition.
* *Clang* catches the common cases (for example the default `throw_exception` operators), but is more conservative through deep call chains and may report a caught violation as a link error against an undefined `+_compile_assert_fail+` symbol rather than a compile error. GCC is the reference toolset for the diagnostic.
