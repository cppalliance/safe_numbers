...
The safe floating-point types `f32` and `f64` compile with NVCC and execute inside device kernels alongside the integer types.
...
As on the host there is no `operator%`, because C++ defines no floating-point remainder operator; see xref:floats.adoc[].

A device kernel uses them the same way it uses the integer types:

[source, c++]
----
using test_type = boost::safe_numbers::f32;

__global__ void scale(const test_type *in, test_type *out, int numElements)
{
    int i = blockDim.x * blockIdx.x + threadIdx.x;

    if (i < numElements)
    {
        out[i] = in[i] * test_type{2.0f};
    }
}
----
...
The mapping mirrors the host behavior of the types documented in xref:floats.adoc[]:
