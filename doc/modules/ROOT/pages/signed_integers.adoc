...
Every width is also available with a saturating or strict error policy in place of the default throwing one: `sat_i8` through `sat_i128` clamp instead of throwing (including `MIN / -1` and negation of `MIN`) and `strict_i8` through `strict_i128` terminate on error.
The `basic_i8<H>` through `basic_i128<H>` alias templates select the policy by type, including user defined handlers: `basic_i32<my_handler>`.
...
using sat_i8   = detail::signed_integer_basis<std::int8_t, overflow_policy::saturate>;
// ... sat_i16, sat_i32, sat_i64, sat_i128

using strict_i8   = detail::signed_integer_basis<std::int8_t, overflow_policy::strict>;
// ... strict_i16, strict_i32, strict_i64, strict_i128

template <signed_integral BasisType, auto ErrorPolicy = overflow_policy::throw_exception>
