...
// -----------------------------------------------
// Valid division: random numerator / random non-zero denominator
// -----------------------------------------------
...
void test_valid_division()
{
    ...
    // Non-zero denominator distribution: [-max, -1] ∪ [1, max]
    ...
    for (std::size_t i {0}; i < N; ++i)
    {
        ...
        // Randomly negate the denominator for full coverage
        if (dist(rng) < 0)
        {
            rhs_value = static_cast<basis_type>(-rhs_value);
        }

        // Skip the min / -1 case (that's tested separately)
...
// -----------------------------------------------
// Division by zero: should throw domain_error
// -----------------------------------------------
...
// -----------------------------------------------
// min / -1 overflow: the only division that overflows
// -----------------------------------------------

template <typename T>
void test_min_div_neg1_overflow()
{
    using basis_type = detail::underlying_type_t<T>;
...
void test_divide_equals()
{
    ...
    // /= with division by zero should throw
    T val {basis_type{42}};
    BOOST_TEST_THROWS(val /= T{basis_type{0}}, std::domain_error);

    // /= with min / -1 should throw overflow
...
void test_boundary_cases()
{
    ...
    // x / x = 1
    BOOST_TEST(T{basis_type{42}} / T{basis_type{42}} == T{basis_type{1}});
    BOOST_TEST(T{basis_type{-42}} / T{basis_type{-42}} == T{basis_type{1}});
    BOOST_TEST(T{std::numeric_limits<basis_type>::max()} / T{std::numeric_limits<basis_type>::max()} == T{basis_type{1}});
    BOOST_TEST(T{std::numeric_limits<basis_type>::min()} / T{std::numeric_limits<basis_type>::min()} == T{basis_type{1}});

    // max / min = 0 (since |max| < |min|, integer division truncates to 0)
    // For two's complement: max = 2^(N-1) - 1, min = -2^(N-1), so max / min = 0
    BOOST_TEST(T{std::numeric_limits<basis_type>::max()} / T{std::numeric_limits<basis_type>::min()} == T{basis_type{0}});

    // min / max = -1 (since min = -(max+1), so min/max = -(max+1)/max = -1 with remainder -1)
    ...
    BOOST_TEST(T{basis_type{7}} / T{basis_type{2}} == T{basis_type{3}});
    BOOST_TEST(T{basis_type{-7}} / T{basis_type{2}} == T{basis_type{-3}});
    BOOST_TEST(T{basis_type{7}} / T{basis_type{-2}} == T{basis_type{-3}});
    BOOST_TEST(T{basis_type{-7}} / T{basis_type{-2}} == T{basis_type{3}});
}
...
int main()
{
    test_valid_division<i8>();
    test_valid_division<i16>();
    test_valid_division<i32>();
    test_valid_division<i64>();
    test_valid_division<i128>();

    test_division_by_zero<i8>();
    test_division_by_zero<i16>();
    test_division_by_zero<i32>();
    test_division_by_zero<i64>();
    test_division_by_zero<i128>();

    test_min_div_neg1_overflow<i8>();
    test_min_div_neg1_overflow<i16>();
    test_min_div_neg1_overflow<i32>();
    test_min_div_neg1_overflow<i64>();
    test_min_div_neg1_overflow<i128>();

    test_divide_equals<i8>();
    test_divide_equals<i16>();
    test_divide_equals<i32>();
    test_divide_equals<i64>();
    test_divide_equals<i128>();
