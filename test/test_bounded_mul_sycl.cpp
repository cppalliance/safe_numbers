...
//
//  In-range bounded multiplication on the device for all three bounded families,
...
int main()
{
    ...
    rc |= sn_sycl_test::run_binary_2<bu>("bounded_uint mul",
        [](std::mt19937_64& rng) { return draw<bu, std::uint32_t>(rng, 0, 1000); },
        [](std::mt19937_64& rng) { return draw<bu, std::uint32_t>(rng, 0, 1000); },
        op);

    rc |= sn_sycl_test::run_binary_2<bi>("bounded_int mul",
        [](std::mt19937_64& rng) { return draw<bi, std::int32_t>(rng, -1000, 1000); },
        [](std::mt19937_64& rng) { return draw<bi, std::int32_t>(rng, -1000, 1000); },
        op);

    rc |= sn_sycl_test::run_binary_2<bf>("bounded_float mul",
        [](std::mt19937_64& rng) { return draw<bf, float>(rng, 0, 1000); },
        [](std::mt19937_64& rng) { return draw<bf, float>(rng, 0, 1000); },
        op);
