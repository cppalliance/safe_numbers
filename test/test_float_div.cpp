...
// -----------------------------------------------
// No error: regular finite division
// IEEE 754 section 6 hot path
// -----------------------------------------------

template <typename T>
void test_finite_division()
{
    using basis_type = typename T::basis_type;

    BOOST_TEST(T{static_cast<basis_type>(6.0)} / T{static_cast<basis_type>(2.0)} == T{static_cast<basis_type>(3.0)});
    ...
    BOOST_TEST(T{static_cast<basis_type>(1.0)} / T{static_cast<basis_type>(2.0)} == T{static_cast<basis_type>(0.5)});
    BOOST_TEST(T{static_cast<basis_type>(-6.0)} / T{static_cast<basis_type>(2.0)} == T{static_cast<basis_type>(-3.0)});
    BOOST_TEST(T{static_cast<basis_type>(-6.0)} / T{static_cast<basis_type>(-2.0)} == T{static_cast<basis_type>(3.0)});
    BOOST_TEST(T{static_cast<basis_type>(6.0)} / T{static_cast<basis_type>(-2.0)} == T{static_cast<basis_type>(-3.0)});

    // Division by one is identity for any finite value
    ...
    // Zero divided by any finite non-zero value is zero
    BOOST_TEST(T{static_cast<basis_type>(0.0)} / T{std::numeric_limits<basis_type>::max()} == T{static_cast<basis_type>(0.0)});
    BOOST_TEST(T{static_cast<basis_type>(0.0)} / T{std::numeric_limits<basis_type>::lowest()} == T{static_cast<basis_type>(0.0)});
    ...
    BOOST_TEST(T{static_cast<basis_type>(1.0)} / pos_inf == T{static_cast<basis_type>(0.0)});
    BOOST_TEST(T{static_cast<basis_type>(1.0)} / neg_inf == T{static_cast<basis_type>(0.0)});
    BOOST_TEST(T{std::numeric_limits<basis_type>::max()} / pos_inf == T{static_cast<basis_type>(0.0)});
...
void test_overflow_to_positive_infinity()
{
    ...
    // max / 0.5 = 2 * max -> +inf
    BOOST_TEST_THROWS(max_val / T{static_cast<basis_type>(0.5)}, std::overflow_error);

    // lowest / -0.5 = (-max) / (-0.5) = 2 * max -> +inf
    BOOST_TEST_THROWS(lowest_val / T{static_cast<basis_type>(-0.5)}, std::overflow_error);

    // max / denorm_min -> +inf
    const T tiny {std::numeric_limits<basis_type>::denorm_min()};
    BOOST_TEST_THROWS(max_val / tiny, std::overflow_error);

    // +inf / finite_positive = +inf -> overflow
    const T pos_inf {std::numeric_limits<basis_type>::infinity()};
    const T neg_inf {-std::numeric_limits<basis_type>::infinity()};
    ...
    // -inf / finite_negative = +inf -> overflow
    BOOST_TEST_THROWS(neg_inf / finite_neg, std::overflow_error);

    // +inf / 0 = +inf -> overflow (per IEEE, no divideByZero for inf dividend)
    const T pos_zero {static_cast<basis_type>(0.0)};
    BOOST_TEST_THROWS(pos_inf / pos_zero, std::overflow_error);

    // -inf / -0 = +inf -> overflow
    const T neg_zero {static_cast<basis_type>(-0.0)};
    BOOST_TEST_THROWS(neg_inf / neg_zero, std::overflow_error);
}
...
void test_underflow_to_negative_infinity()
{
    ...
    // lowest / 0.5 = -max / 0.5 = -2 * max -> -inf
    BOOST_TEST_THROWS(lowest_val / T{static_cast<basis_type>(0.5)}, std::underflow_error);

    // max / -0.5 = -2 * max -> -inf
    BOOST_TEST_THROWS(max_val / T{static_cast<basis_type>(-0.5)}, std::underflow_error);

    // lowest / denorm_min -> -inf
    const T tiny {std::numeric_limits<basis_type>::denorm_min()};
    BOOST_TEST_THROWS(lowest_val / tiny, std::underflow_error);

    // +inf / finite_negative = -inf -> underflow
    const T pos_inf {std::numeric_limits<basis_type>::infinity()};
    const T neg_inf {-std::numeric_limits<basis_type>::infinity()};
    const T finite_pos {static_cast<basis_type>(1.0)};
    const T finite_neg {static_cast<basis_type>(-1.0)};
    ...
    // -inf / finite_positive = -inf -> underflow
    BOOST_TEST_THROWS(neg_inf / finite_pos, std::underflow_error);

    // +inf / -0 = -inf -> underflow (per IEEE, no divideByZero for inf dividend)
    const T neg_zero {static_cast<basis_type>(-0.0)};
    BOOST_TEST_THROWS(pos_inf / neg_zero, std::underflow_error);

    // -inf / +0 = -inf -> underflow
    const T pos_zero {static_cast<basis_type>(0.0)};
    BOOST_TEST_THROWS(neg_inf / pos_zero, std::underflow_error);
}
...
void test_qnan_operand()
{
    ...
    // QNaN / zero and zero / QNaN (NaN dividend takes priority over divide-by-zero)
    BOOST_TEST_THROWS(qnan / zero, std::domain_error);
    BOOST_TEST_THROWS(zero / qnan, std::domain_error);
}

// -----------------------------------------------
// Invalid op: zero divided by zero
// IEEE 754 section 7.2.b
// (0/0) yields NaN regardless of sign
// -----------------------------------------------

template <typename T>
void test_zero_div_zero_invalid()
{
    using basis_type = typename T::basis_type;
...
// -----------------------------------------------
// Invalid op: infinity divided by infinity
// IEEE 754 section 7.2.b
// (inf/inf) yields NaN regardless of sign
// -----------------------------------------------

template <typename T>
void test_inf_div_inf_invalid()
{
    using basis_type = typename T::basis_type;
...
// -----------------------------------------------
// Division by zero: finite non-zero dividend / zero
// IEEE 754 section 7.3
// Produces signed infinity and signals divideByZero
// -----------------------------------------------
...
int main()
{
    test_finite_division<f32>();
    test_finite_division<f64>();
    ...
    test_zero_div_zero_invalid<f32>();
    test_zero_div_zero_invalid<f64>();

    test_inf_div_inf_invalid<f32>();
    test_inf_div_inf_invalid<f64>();

    test_divide_by_zero<f32>();
    test_divide_by_zero<f64>();
