...
//
//  An unsigned multiplication that overflows must be reported from the device and surface on the host as
...
int run(const char* label)
{
    ...
    return sn_sycl_test::expect_binary_throw<std::overflow_error, T>(label,
        T{(std::numeric_limits<basis_type>::max)()}, T{basis_type{2}},
