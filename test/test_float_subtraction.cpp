...
// -----------------------------------------------
// No error: regular finite subtraction
// IEEE 754 section 6 hot path
// -----------------------------------------------

template <typename T>
void test_finite_subtraction()
{
    using basis_type = typename T::basis_type;

    BOOST_TEST(T{static_cast<basis_type>(3.0)} - T{static_cast<basis_type>(2.0)} == T{static_cast<basis_type>(1.0)});
    ...
    BOOST_TEST(T{static_cast<basis_type>(0.5)} - T{static_cast<basis_type>(0.5)} == T{static_cast<basis_type>(0.0)});
    BOOST_TEST(T{static_cast<basis_type>(-3.5)} - T{static_cast<basis_type>(-1.5)} == T{static_cast<basis_type>(-2.0)});
    BOOST_TEST(T{static_cast<basis_type>(-1.0)} - T{static_cast<basis_type>(1.0)} == T{static_cast<basis_type>(-2.0)});

    // Subtracting zero is identity for any finite value
    ...
    // x - x cancels exactly to zero for any finite x
    BOOST_TEST(T{std::numeric_limits<basis_type>::max()} - T{std::numeric_limits<basis_type>::max()} == T{static_cast<basis_type>(0.0)});
...
void test_overflow_to_positive_infinity()
{
    ...
    BOOST_TEST_THROWS(max_val - lowest_val, std::overflow_error);

    // Two large finite values whose difference overflows past max
    const T big {std::numeric_limits<basis_type>::max() / static_cast<basis_type>(2)};
    const T big_neg {std::numeric_limits<basis_type>::lowest()};
    BOOST_TEST_THROWS(big - big_neg, std::overflow_error);

    // +inf - finite is +inf -> overflow
    ...
    // finite - (-inf) is +inf -> overflow
    BOOST_TEST_THROWS(finite - neg_inf, std::overflow_error);

    // +inf - (-inf) is +inf -> overflow
    BOOST_TEST_THROWS(pos_inf - neg_inf, std::overflow_error);
}
...
void test_underflow_to_negative_infinity()
{
    ...
    // lowest - max = -max - max = -inf
    BOOST_TEST_THROWS(lowest_val - max_val, std::underflow_error);

    // Two large opposite-sign finite values whose difference underflows
    const T big_neg {std::numeric_limits<basis_type>::lowest() / static_cast<basis_type>(2)};
    const T big_pos {std::numeric_limits<basis_type>::max()};
    BOOST_TEST_THROWS(big_neg - big_pos, std::underflow_error);

    // -inf - finite is -inf -> underflow
    const T pos_inf {std::numeric_limits<basis_type>::infinity()};
    const T neg_inf {-std::numeric_limits<basis_type>::infinity()};
    const T finite {static_cast<basis_type>(1.0)};
    BOOST_TEST_THROWS(neg_inf - finite, std::underflow_error);

    // finite - (+inf) is -inf -> underflow
    BOOST_TEST_THROWS(finite - pos_inf, std::underflow_error);

    // -inf - (+inf) is -inf -> underflow
...
// -----------------------------------------------
// Invalid op: subtraction of same-sign infinities
// IEEE 754 section 7.2.d
// (+inf - +inf) and (-inf - -inf) cancel to NaN
// -----------------------------------------------

template <typename T>
void test_same_sign_inf_invalid()
{
    using basis_type = typename T::basis_type;
...
int main()
{
    test_finite_subtraction<f32>();
    test_finite_subtraction<f64>();
    ...
    test_same_sign_inf_invalid<f32>();
    test_same_sign_inf_invalid<f64>();
