...
//
//  In-range bounded addition on the device for all three bounded families,
...
int main()
{
    ...
    rc |= sn_sycl_test::run_binary_2<bu>("bounded_uint add",
        [](std::mt19937_64& rng) { return draw<bu, std::uint32_t>(rng, 0, 400000); },
        [](std::mt19937_64& rng) { return draw<bu, std::uint32_t>(rng, 0, 400000); },
        op);

    rc |= sn_sycl_test::run_binary_2<bi>("bounded_int add",
        [](std::mt19937_64& rng) { return draw<bi, std::int32_t>(rng, -400000, 400000); },
        [](std::mt19937_64& rng) { return draw<bi, std::int32_t>(rng, -400000, 400000); },
        op);

    rc |= sn_sycl_test::run_binary_2<bf>("bounded_float add",
        [](std::mt19937_64& rng) { return draw<bf, float>(rng, 0, 400000); },
        [](std::mt19937_64& rng) { return draw<bf, float>(rng, 0, 400000); },
        op);
