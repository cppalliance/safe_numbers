...
void test_valid_subtraction()
{
    ...
    boost::random::uniform_int_distribution<basis_type> dist {static_cast<basis_type>(std::numeric_limits<basis_type>::min() / 4),
                                                              static_cast<basis_type>(std::numeric_limits<basis_type>::max() / 4)};
    ...
    for (std::size_t i {0}; i < N; ++i)
    {
        ...
        const auto [res, overflowed] = overflowing_sub(lhs, rhs);
...
int main()
{
    ...
    test_overflowed_positive<i8>();
    test_overflowed_negative<i8>();

    test_valid_subtraction<i16>();
    test_overflowed_positive<i16>();
    test_overflowed_negative<i16>();

    test_valid_subtraction<i32>();
    test_overflowed_positive<i32>();
    test_overflowed_negative<i32>();

    test_valid_subtraction<i64>();
    test_overflowed_positive<i64>();
    test_overflowed_negative<i64>();

    test_valid_subtraction<i128>();
    test_overflowed_positive<i128>();
    test_overflowed_negative<i128>();
