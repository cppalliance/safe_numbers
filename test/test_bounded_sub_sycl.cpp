...
//
//  In-range bounded subtraction on the device for all three bounded families,
...
int main()
{
    ...
    rc |= sn_sycl_test::run_binary_2<bu>("bounded_uint sub",
        [](std::mt19937_64& rng) { return draw<bu, std::uint32_t>(rng, 500000, 1000000); },
        [](std::mt19937_64& rng) { return draw<bu, std::uint32_t>(rng, 0, 500000); },
        op);

    rc |= sn_sycl_test::run_binary_2<bi>("bounded_int sub",
        [](std::mt19937_64& rng) { return draw<bi, std::int32_t>(rng, -400000, 400000); },
        [](std::mt19937_64& rng) { return draw<bi, std::int32_t>(rng, -400000, 400000); },
        op);

    rc |= sn_sycl_test::run_binary_2<bf>("bounded_float sub",
        [](std::mt19937_64& rng) { return draw<bf, float>(rng, 500000, 1000000); },
        [](std::mt19937_64& rng) { return draw<bf, float>(rng, 0, 500000); },
        op);
