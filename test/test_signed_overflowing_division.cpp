...
void test_valid_division()
{
    ...
    // Distribute over non-zero values, excluding the min/-1 case
    ...
    for (std::size_t i {0}; i < N; ++i)
    {
        ...
        // Skip the min / -1 edge case (rhs_dist is [1, max] so this won't happen anyway)
...
void test_min_div_neg1()
{
    ...
    BOOST_TEST_EQ(res, T{std::numeric_limits<basis_type>::min()});
    BOOST_TEST(overflowed);
}
