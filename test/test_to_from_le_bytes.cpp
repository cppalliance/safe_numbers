...
// =============================================================================
// Verified overloads: to_le_bytes and from_le_bytes at compile time
// =============================================================================

void test_verified_to_le_bytes()
{
    {
        constexpr auto bytes = to_le_bytes(verified_u8{u8{0x42}});
        static_assert(bytes[0] == std::byte{0x42});
    }
    {
        constexpr auto bytes = to_le_bytes(verified_u16{u16{static_cast<std::uint16_t>(0x0102)}});
        static_assert(bytes[0] == std::byte{0x02});
        static_assert(bytes[1] == std::byte{0x01});
    }
    {
        constexpr auto bytes = to_le_bytes(verified_u32{u32{0x01020304U}});
        static_assert(bytes[0] == std::byte{0x04});
        static_assert(bytes[1] == std::byte{0x03});
        static_assert(bytes[2] == std::byte{0x02});
        static_assert(bytes[3] == std::byte{0x01});
    }
    {
        constexpr auto bytes = to_le_bytes(verified_u64{u64{0x0102030405060708ULL}});
        static_assert(bytes[0] == std::byte{0x08});
        static_assert(bytes[1] == std::byte{0x07});
        static_assert(bytes[2] == std::byte{0x06});
        static_assert(bytes[3] == std::byte{0x05});
        static_assert(bytes[4] == std::byte{0x04});
        static_assert(bytes[5] == std::byte{0x03});
        static_assert(bytes[6] == std::byte{0x02});
        static_assert(bytes[7] == std::byte{0x01});
    }
}

void test_verified_from_le_bytes()
{
    {
        constexpr std::array<std::byte, 1> bytes {std::byte{0x42}};
        constexpr auto val = from_le_bytes<verified_u8>(std::span<const std::byte, 1>{bytes});
    ...
    }
    {
        ...
        constexpr auto val = from_le_bytes<verified_u64>(std::span<const std::byte, 8>{bytes});
        static_assert(static_cast<u64>(val) == u64{0x0102030405060708ULL});
    }
}

void test_verified_le_bytes_round_trip()
{
    {
        constexpr auto original = verified_u32{u32{0xDEADBEEFU}};
        constexpr auto bytes = to_le_bytes(original);
        constexpr auto reconstructed = from_le_bytes<verified_u32>(std::span<const std::byte, 4>{bytes});
...
int main()
{
    ...
    test_verified_to_le_bytes();
    test_verified_from_le_bytes();
    test_verified_le_bytes_round_trip();
