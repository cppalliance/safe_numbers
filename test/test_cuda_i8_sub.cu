...
using test_type = boost::safe_numbers::i8;
...
int main(void)
{
    ...
    std::uniform_int_distribution<std::int32_t> dist{static_cast<std::int32_t>((std::numeric_limits<basis_type>::min)() / 4), static_cast<std::int32_t>((std::numeric_limits<basis_type>::max)() / 4)};
    for (int i = 0; i < numElements; ++i)
    {
        input_vector[i] = test_type{static_cast<basis_type>(dist(rng))};
        input_vector2[i] = test_type{static_cast<basis_type>(dist(rng))};
    }
