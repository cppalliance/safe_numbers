...
void test_valid_subtraction()
{
    ...
    for (std::size_t i = 0; i < N; ++i)
    {
        ...
        const T res {wrapping_sub(lhs, rhs)};
...
template <typename T>
void test_wrapping_subtraction()
{
    using basis_type = detail::underlying_type_t<T>;
    ...
    for (std::size_t i = 0; i < N; ++i)
    {
        ...
        // Calculate expected wrapped result (lhs - rhs wraps around)
        T expected_value {};
        if constexpr (std::is_same_v<basis_type, std::uint8_t> || std::is_same_v<basis_type, std::uint16_t>)
        {
...
int main()
{
    ...
    test_wrapping_subtraction<u8>();
    test_wrapping_subtraction<u16>();
    test_wrapping_subtraction<u32>();
    test_wrapping_subtraction<u64>();
    test_wrapping_subtraction<u128>();
