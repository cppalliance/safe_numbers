...
void test_roundtrip()
{
    ...
    for (int base = 2; base <= 36; ++base)
    {
        ...
        // Test negative values explicitly
        test(T{-42}, base);
        test(T{-1}, base);
...
template <typename T>
void test_roundtrip_positive_only()
{
    using basis_type = detail::underlying_type_t<T>;
    boost::random::uniform_int_distribution<basis_type> dist {basis_type{0},
                                                              std::numeric_limits<basis_type>::max()};
...
int main()
{
    ...
    // i128 from_chars has limited negative value support; test positive only
    test_roundtrip_positive_only<i128>();
