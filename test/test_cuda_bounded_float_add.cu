...
using test_type = boost::safe_numbers::bounded_float<0.0f, 1000000.0f>;
...
int main()
{
    ...
    std::uniform_real_distribution<float> dist{0.0f, 400000.0f};
    for (int i = 0; i < numElements; ++i)
    {
        A[i] = test_type{basis_type{dist(rng)}};
        B[i] = test_type{basis_type{dist(rng)}};
    }
